...
# --- XỬ LÝ LOGIC TRẢ THƯỞNG ---
def handle_payout(match_id, result):
    conn = get_conn()
    # Tìm người thắng
    winners = conn.execute(f"SELECT username, bet_1x2 FROM predictions WHERE match_id={match_id} AND predicted_1x2='{result}'").fetchall()
    for uname, bet in winners:
        conn.execute("UPDATE users SET points = points + ? WHERE username = ?", (bet * 2, uname))
    conn.execute(f"UPDATE matches SET status='closed', actual_result='{result}' WHERE id={match_id}")
    conn.commit(); conn.close()
...
if "username" not in st.session_state:
    ...
    menu = st.sidebar.radio("Chọn:", ["🎮 Lên kèo", "📊 Phiếu cược", "🔑 Đổi mã PIN", "⚙️ Admin Hub" if user[2]=='admin' else ""])
    ...
    if menu == "🎮 Lên kèo":
        ...
        for _, m in matches.iterrows():
            with st.expander(f"⚽ {m['match_name']} ({m['group_name']})"):
                ...
                sc = c1.text_input("Tỉ số:", key=f"s_{m['id']}")
        ...
        with t2:
            ...
            if not df_m.empty:
                ...
                if st.button("UNDO"):
                    conn.execute(f"UPDATE matches SET status='open' WHERE id={m_id}")
