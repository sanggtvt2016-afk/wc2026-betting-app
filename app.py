...
# =========================================================================
# 1. CẤU HÌNH & KẾT NỐI (SỬ DỤNG DB DUY NHẤT)
# =========================================================================
...
if "username" not in st.session_state:
    ...
    menu = st.sidebar.radio("Chọn:", ["🎮 Lên kèo", "📊 Thống kê cá nhân", "⚙️ Admin Hub" if user[2]=='admin' else ""])
    ...
    if menu == "🎮 Lên kèo":
        ...
        for _, m in matches.iterrows():
            with st.expander(f"⚽ {m['match_name']} - {m['group_name']}"):
                c1, c2, c3 = st.columns(3)
                opt = c1.radio("Chọn kết quả:", m['options'].split(','), key=f"o_{m['id']}")
                b1 = c2.number_input("Cược Kết Quả (x2):", 0, step=10, key=f"b1_{m['id']}")
    ...
    # TRANG ADMIN HUB
        ...
        with t1:
            name = st.text_input("Tên trận (VD: Brazil vs Pháp):")
        ...
        with t2:
            ...
            if not df_m.empty: # --- DÒNG KIỂM TRA ĐỂ TRÁNH LỖI LINE 40 ---
                ...
                if st.button("Chốt & Trả thưởng"):
                    ...
                    # Logic trả thưởng đơn giản
                    conn.execute(f"UPDATE users SET points = points + 100 WHERE username IN (SELECT username FROM predictions WHERE predicted_1x2='{res}' AND match_id=(SELECT id FROM matches WHERE match_name='{match}'))")
            ...
            else: st.info("Chưa có trận nào để chốt.")
